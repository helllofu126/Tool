...
{
    public class UdpServerCustom : ProtocolBase
    {
        public UdpServerCustom(int localPort) : base(localPort)
        {
        ...
        private void _Thread_RecvMessage()
        {
            ...
            while (false == _isThreadExitEnabled)
            {
                ...
                while (false == _isThreadExitEnabled)
                {
                    try
                    {
                        var byBuff = _udpServer.Receive(ref remoteIpEndPoint);
        ...
        private void _Thread_SendMessage()
        {
            ...
            while (false == _isThreadExitEnabled)
            {
                try
                {
                    ...
                    //发送数据
                    _udpServer.Send(bySend, bySend.Length, RemoteIpEndPoint);
