...
namespace HTTPServerLib
{
    ...
    public class HttpRequest : BaseHeader
    {
        ...
        public HttpRequest(Stream stream)
        {
            ...
            var contentLengthString = GetHeader(RequestHeaders.ContentLength);
            if (int.TryParse(contentLengthString, out var contentLength) && bodyDataLength < contentLength)
            {
                int tempLength = 0;
                do
                {
                    byte[] bytes = new byte[MAX_SIZE];
                    tempLength = stream.Read(bytes, 0, MAX_SIZE - 1);
                    Body += Encoding.UTF8.GetString(bytes, 0, tempLength);
                    bodyDataLength += tempLength;
                } while (bodyDataLength < contentLength && tempLength > 0);
            }
